...
number_riders = 80 # Number of riders in the system
number_drivers = 48 # Number of drivers in the system
...
run_time = 280 # Running time of a single run of RHS emulation. With more time, more data will be collected
...
exp_folder='./planar_lap_diff_model_r_1.0'
	...
	while eta_tolerance < max_eta_tolerance+1:
		for mech_number, mech_name in enumerate(mechanisms):
			for i in range(0, len(genric_utilities)):
				...
				x.append(config[0][0]+config[1][0]+config[2][0])
		eta_tolerance += 200
	eta_tolerance=400
...
exp_folder='./planar_lap_diff_model_r_0.5'
	...
	while eta_tolerance < max_eta_tolerance+1:
		for mech_number, mech_name in enumerate(mechanisms):
			for i in range(0, len(genric_utilities)):
				...
				# x_.append(genric_utilities[i])
				tail_ql_data_means_9.append(np.mean(np.array(tail_ql)))
				gen_ql_data_means_9.append(np.mean(np.array(gen_ql)))

				tail_ql_data_meds_9.append(np.median(np.array(tail_ql)))
				gen_ql_data_meds_9.append(np.median(np.array(gen_ql)))
...
kwargs = dict(capsize=2, elinewidth=0.6, linewidth=1.1, ms=7)
...
plt.xlabel('$\mathsf{M_d}$ of drivers per PRide run', labelpad=15,fontsize=20)
plt.ylabel('Tailored QL (seconds), Generic QL (meters)', labelpad=15,fontsize=20)

plt.xticks(np.arange(len(tail_ql_data_meds)),['HHH','HHE','HHL','LHM','HMM','MMM','LMM','LLH','LLM','LLL'])

plt.show()

# try:
# 	fig.savefig('planar_lap_diff_driver_models.pdf')
# except:
# 	pass
